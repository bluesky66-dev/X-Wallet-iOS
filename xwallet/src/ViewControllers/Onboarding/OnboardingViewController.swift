...
class OnboardingViewController: UIViewController {
    ...
    private let logoImageView: UIImageView = UIImageView(image: UIImage(named: "XLogo"))
    var introImageView: [UIImageView] = [UIImageView]()
//    private let dotImageView: UIImageView = UIImageView(frame: CGRect(x: 100, y: 150, width: 150, height: 150))
//    private let cryptoIconImageView: UIImageView = UIImageView(frame: CGRect(x: 100, y: 150, width: 150, height: 150))
    private let dotImageView: UIImageView = UIImageView(image: UIImage(named: "DotImage"))
    private let cryptoIconImageView: UIImageView = UIImageView(image: UIImage(named: "CryptoIcons"))
    ...
    private var showingDotImage: Bool = false {
        didSet {
            let alpha: CGFloat = showingDotImage ? 1 : 0
            UIView.animate(withDuration: 0.3, delay: 0, options: .curveEaseInOut, animations: {
                self.logoImageView.alpha = alpha
            }, completion: nil)
        }
...
//    }
    ...
    var isPaging: Bool = false {
        didSet {
            [bottomButton, skipButton, backButton].forEach { (button) in
...
//
//        logoImageView.frame = CGRect(x: 0, y: 0, width: screensize.width * 0.8, height: screensize.height * 0.4)
...
//            return UIFont.onboardingSmallHeading()
    ...
    private func setUpHeadingLabels() {
        ...
        for (index, page) in pages.enumerated() {
            ...
            headingLabel.constrain([animationConstraint, leading, trailing, headingLabel.centerXAnchor.constraint(equalTo: view.centerXAnchor, constant: 0),
                headingLabel.centerYAnchor.constraint(equalTo: view.centerYAnchor, constant: -(screensize.height/4+20))
            ])
            ...
            animationConstraint = subheadingLabel.topAnchor.constraint(equalTo: headingLabel.bottomAnchor, constant: headingSubheadingMargin)
            ...
            subheadingLabel.constrain([animationConstraint, leading, trailing, headingLabel.centerXAnchor.constraint(equalTo: view.centerXAnchor, constant: 0),
                headingLabel.centerYAnchor.constraint(equalTo: view.centerYAnchor, constant: -(screensize.height/4 + 20))
            ])
...
//                return S.CloudBackup.restoreButton
    ...
    private func setUpBottomButtons() {
        ...
        middleButton.layer.cornerRadius = 10.0
        ...
        bottomButton.backgroundColor = Theme.secondaryButtonBackground
        middleButton.backgroundColor = Theme.primaryButtonBackground
    ...
    private func setUpIntroImages() {
//        dotImageView.image = UIImage(named: "DotImage")
//        cryptoIconImageView.image = UIImage(named: "CryptoIcons")
        dotImageView.alpha = 0
        cryptoIconImageView.alpha = 0
        
        self.view.addSubview(dotImageView)
        self.view.addSubview(cryptoIconImageView)
        
        let screensize = UIScreen.main.bounds
        let offset = screensize.height / 8.0
        
        dotImageView.frame.size.width = screensize.width
        dotImageView.constrain([
        dotImageView.centerXAnchor.constraint(equalTo: view.centerXAnchor, constant: 0),
            dotImageView.centerYAnchor.constraint(equalTo: view.centerYAnchor, constant: offset * 2 - 30.0)
        ])
//        dotImageView.contentMode = .scaleToFill
        dotImageView.contentMode = .scaleAspectFit
        
        cryptoIconImageView.constrain([
        cryptoIconImageView.centerXAnchor.constraint(equalTo: view.centerXAnchor, constant: 0),
        cryptoIconImageView.centerYAnchor.constraint(equalTo: view.centerYAnchor, constant: offset)
        ])
//        cryptoIconImageView.contentMode = .scaleToFill
        cryptoIconImageView.contentMode = .scaleAspectFit
        
//        let cryptoImageSize = cryptoIconImageView.image?.size
//        let rect = CGRect(x: 0, y: 0, width: screensize.width, height: screensize.height * 0.3)
//        cryptoIconImageView.image?.draw(in: rect)
        introImageView.append(dotImageView)
        introImageView.append(cryptoIconImageView)
    }
...
//        startVideoClipForPage(pageIndex: nextIndex)
        ...
        let firstIntroImage = introImageView[0]
        let secondIntroImage = introImageView[1]
        if pageIndex == 0 {
            firstIntroImage.alpha = 1
            secondIntroImage.alpha = 0
        } else if pageIndex == 1 {
            firstIntroImage.alpha = 0
            secondIntroImage.alpha = 1
        } else {
            firstIntroImage.alpha = 0
            secondIntroImage.alpha = 0
        }
