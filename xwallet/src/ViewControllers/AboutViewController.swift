...
//    private let logo = UIImageView(image: #imageLiteral(resourceName: "LogoCutout").withRenderingMode(.alwaysTemplate))
    private var logo = UIImageView(image: UIImage(named: "XLarge"))
...
//            walletID.trailingAnchor.constraint(equalTo: view.trailingAnchor) ])
        blog.constrain([
            blog.topAnchor.constraint(equalTo: logoBackground.bottomAnchor, constant: verticalMargin),
    ...
    private func setActions() {
        blog.button.tap = strongify(self) { myself in
            myself.presentURL(string: "https://xwallet.com/blog/")
        }
        twitter.button.tap = strongify(self) { myself in
            myself.presentURL(string: "https://twitter.com/xwallethq")
        }
        reddit.button.tap = strongify(self) { myself in
            myself.presentURL(string: "https://reddit.com/r/xwalletapp/")
        }
        privacy.tap = strongify(self) { myself in
            myself.presentURL(string: "https://xwallet.com/privacy")
        }
    }
