...
extension CurrencyMetaData: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        if colorValues.count == 2 {
            if uid == "bitcoin-mainnet:__native__" {
                colors = (UIColor.fromHex("#0791ff"), UIColor.fromHex("#0791ff"))
            } else if uid == "ethereum-mainnet:__native__" {
                colors = (UIColor.fromHex("#FF5454"), UIColor.fromHex("#FF5454"))
            } else {
